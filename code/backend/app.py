...
API_KEY = "XeNHiK6pLDHE2MYxOyW5bOmv01ZN73oy"  # Inserisci la tua chiave TomTom
...
def update_position():
    ...
    current_positions[session_id] = {"lat": data["lat"], "lon": data["lon"]}
...
def route_info():
    ...
    end_coords = geocode_address(end_addr)
    if not start_coords or not end_coords:
        return jsonify({"error": "Indirizzi non validi"}), 400
